...
function Model({ color, design, side, transform, viewScale, viewRotation, onDesignPointerDown }) {
...
function Shirt3D({ color, design, side, transform, onDesignPointerDown }) {
  ...
  const startInteraction = (event) => {
    if (event.target.closest('button') || interaction.current === 'design') return
    event.currentTarget.setPointerCapture(event.pointerId)
    interaction.current = { x: event.clientX, rotation: viewRotation }
  }
  const moveInteraction = (event) => {
    if (!interaction.current || interaction.current === 'design') return
    const width = event.currentTarget.getBoundingClientRect().width
    const nextRotation = interaction.current.rotation + ((event.clientX - interaction.current.x) / width) * 1.5
    setViewRotation(Math.max(-0.5, Math.min(0.5, nextRotation)))
  }
  const endInteraction = () => { interaction.current = null }
  const startDesignInteraction = (event) => {
    interaction.current = 'design'
    onDesignPointerDown?.(event)
  }

  return (
    <div className="shirt-3d" aria-label={`Modelo 3D de remera ${color.name}, vista ${side}`} onPointerDown={startInteraction} onPointerMove={moveInteraction} onPointerUp={endInteraction} onPointerCancel={endInteraction}>
      <Canvas camera={{ position: [0, 0.15, 5.4], fov: 26 }} dpr={[1, 2]} gl={{ antialias: true, alpha: true }}>
        ...
        <Suspense fallback={null}>
          <Model color={color} design={design} side={side} transform={transform} viewScale={viewScale} viewRotation={viewRotation} onDesignPointerDown={startDesignInteraction} />
      ...
      <div className="view-zoom" aria-label="Tamaño de la vista de la remera">
        <button type="button" onPointerDown={(event) => event.stopPropagation()} onClick={zoomOut} disabled={viewScale <= 0.75} aria-label="Alejar remera">−</button>
        <span>{Math.round(viewScale * 100)}%</span>
        <button type="button" onPointerDown={(event) => event.stopPropagation()} onClick={zoomIn} disabled={viewScale >= 1.25} aria-label="Acercar remera">+</button>
      </div>
    </div>
  )
}
