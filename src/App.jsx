...
function App() {
  ...
  useEffect(() => {
    ...
    products.forEach((product) => {
      ...
      const category = document.createElement('span'); const name = document.createElement('h3'); const priceLine = document.createElement('div'); const price = document.createElement('strong'); const link = document.createElement('a')
      category.textContent = product.category || 'Remeras'; name.textContent = product.title; price.textContent = `$ ${Number(product.price || 0).toLocaleString('es-AR')}`; link.href = '/personaliza'; link.innerHTML = 'Personalizar <b>→</b>'
      ...
      priceLine.append(price, link); info.append(priceLine); card.append(imageBox, info); grid.append(card)
    })
